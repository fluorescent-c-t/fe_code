...
def variables_gp(De_gp, D_gp, dε_gp, dεvp_gp, dd, Bf, B1, σA_gp):
    for i in range(gl.num_element):
        dde = np.zeros(((gl.dof_m + 1) * gl.num_node_quad1, 1))
        for j in range(gl.num_node_quad1):
            dde[(gl.dof_m + 1) * j    ] = dd[gl.dof_m * (gl.conn[i][j] - 1)    ]
        ...
        for j in range(gl.num_gp):
            ...
            σa = np.zeros((gl.num_strn_cmp, gl.num_dfmgrd_cmp))
            
            σa[0][0],  σa[1][1], σa[2][2] = 2.0 * sigxx, 2.0 * sigyy, 2.0 * sigzz
            σa[3][3],  σa[3][4]           = 1.0 * sigzz, 1.0 * sigxx
            σa[3][0] = σa[3][2]           = 1.0 * tauzx
            σa[0][3] = σa[2][4]           = 2.0 * tauzx
            
            dsig = D_gp[i][j] @ (dε_gp[i][j] - dεvp_gp[i][j]) + σa @ (- Bf[i][j]) @ dde - σA_gp[i][j] @ (-B1[i][j]) @ dde
            ...
            εnv = gl.strain_inv_gp[i][j][0]
            de  = (1. - np.exp(-εnv)) * (1. + e0)
...
def coordinate(solution_dd):
    dd = np.array([[solution_dd[gl.dof_m * i][0], solution_dd[gl.dof_m  * i + 1][0]] for i in range(gl.num_node)])
    gl.global_coord += dd
