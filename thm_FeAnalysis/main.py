...
#Read geometry
detJ, B1, B2, Bv = sf.mtrcs_deriv_gp()
...
for timestep in range(num_timestep):
    ...
    De_gp   = np.zeros((gl.num_element, gl.num_gp, gl.num_strn_cmp, gl.num_strn_cmp))#Mechanical constitutive rel. for the state updates later
    ...
    for i in range(gl.num_element): 
        K1e, rx_e = cm.matrcs_ele(detJ, B1, i, T_gp, dt, De_gp, D_gp, dεvp_gp)#Form element matrices
    ...
    #Update
    updt.variables_gp(De_gp, D_gp, dε_gp, dεvp_gp)
